Compiled 1000 times in 6.104 s  |  6.104 ms/compile  |  57.6 KB code  (7 partials)
Executed 1000 times in 2.571 s  |  2.571 ms/render   |  139.9 KB output
<?php
...
use \LightnCandy\SafeString as SafeString;use \LightnCandy\Runtime as LR;return function ($in = null, $options = null) {
    $helpers = array(            't' => function(string $key, $options) {
        ...
        return $a == $b;
            ...
            'and' => function(mixed $a, mixed $b) { return $a && $b; },
            'not' => function(mixed $a) { return !$a; },
            'gt' => function(mixed $a, mixed $b) { return $a > $b; },
);
    $partials = array();
    $cx = array(
        'flags' => array(
...
'.((LR::ifvar($cx, (($inary && isset($in['user'])) ? $in['user'] : null), false)) ? '            <div class="user-menu">
                <img src="'.LR::encq($cx, ((isset($in['user']) && is_array($in['user']) && isset($in['user']['avatar'])) ? $in['user']['avatar'] : null)).'" alt="'.LR::encq($cx, ((isset($in['user']) && is_array($in['user']) && isset($in['user']['name'])) ? $in['user']['name'] : null)).'" width="32" height="32">
                <span>'.LR::encq($cx, ((isset($in['user']) && is_array($in['user']) && isset($in['user']['name'])) ? $in['user']['name'] : null)).'</span>
                <ul class="user-dropdown">
                    <li><a href="/profile/'.LR::encq($cx, ((isset($in['user']) && is_array($in['user']) && isset($in['user']['id'])) ? $in['user']['id'] : null)).'">'.LR::encq($cx, LR::hbch($cx, 't', array(array('nav.profile'),array()), 'encq', $in)).'</a></li>
                    <li><a href="/settings">'.LR::encq($cx, LR::hbch($cx, 't', array(array('nav.settings'),array()), 'encq', $in)).'</a></li>
                    '.((LR::ifvar($cx, ((isset($in['user']) && is_array($in['user']) && isset($in['user']['isAdmin'])) ? $in['user']['isAdmin'] : null), false)) ? '<li><a href="/admin">'.LR::encq($cx, LR::hbch($cx, 't', array(array('nav.admin'),array()), 'encq', $in)).'</a></li>' : '').'
                    <li class="divider"></li>
                    <li><a href="/logout">'.LR::encq($cx, LR::hbch($cx, 't', array(array('nav.logout'),array()), 'encq', $in)).'</a></li>
                </ul>
            </div>
' : '            <a class="btn btn-primary" href="/login">'.LR::encq($cx, LR::hbch($cx, 't', array(array('nav.login'),array()), 'encq', $in)).'</a>
').'    </nav>
</header>
...
'.'        <div class="page-header">
            <h1>
                ...
                '.((LR::ifvar($cx, (($inary && isset($in['headingBadge'])) ? $in['headingBadge'] : null), false)) ? '<span class="badge badge-'.LR::encq($cx, ((isset($in['headingBadge']) && is_array($in['headingBadge']) && isset($in['headingBadge']['type'])) ? $in['headingBadge']['type'] : null)).'">'.LR::encq($cx, ((isset($in['headingBadge']) && is_array($in['headingBadge']) && isset($in['headingBadge']['text'])) ? $in['headingBadge']['text'] : null)).'</span>' : '').'
            </h1>
